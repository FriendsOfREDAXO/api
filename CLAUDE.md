...
- **Noch offen:** Das Plugin verweigert Usern ohne `version[live_version]` das Schreiben in die Live-Version (`version/boot.php`). Die API prüft dieses Recht nicht — ein Backend-User kann über `/api/backend/...` in Live schreiben, obwohl das Backend es ihm verwehrt.
