...
**OpenAPI-Generator (`OpenAPIConfig`):** Alle Feld-Definitionen — `query` wie `Body` — laufen über `getSchema()`/`getSchemaType()`: die Typnamen der Route-Definitionen (`int`, `bool`, `float`) werden auf OpenAPI-Typen gemappt, Typ und Default gehören ins `schema`, nicht an den Parameter. `required` steht als Liste auf Objektebene, nie als Bool in der Property, und ein leeres `required` wird weggelassen. `'type' => 'file'` ergänzt `format: binary` und schaltet den Content-Type auf `multipart/form-data`. Tags ohne Sprachschlüssel (`api_openapi_tag_<tag>_description`) bekommen eine leere Beschreibung statt eines `[translate:…]`-Platzhalters — Tags können aus fremden AddOns kommen. **Tags sind die Accordion-Gruppen in Swagger UI** und werden alphabetisch sortiert ausgegeben, damit die Reihenfolge nicht von der Registrierungsreihenfolge der RoutePackages abhängt. Gibt eine Route keinen eigenen Tag mit, leitet `RouteCollection::tagFromScope()` ihn aus dem ersten Scope-Segment ab (`media/category/add` → `media`); der Scope selbst wäre als Gruppe unbrauchbar, weil fast jeder zu genau einem Endpunkt gehört. Die Backend-Spiegel nehmen über `RouteCollection::backendTag()` die Gruppe der gespiegelten Route und stellen `backend/` davor — nicht neu aus dem Scope abgeleitet, sonst weicht der Spiegel bei Routen mit eigenem Tag ab (die Selbstauskunft heißt im Scope `me`, als Tag aber `meta`). `tags` muss ein Array bleiben (`array_values`), sonst ist das Dokument für getypte Parser ungültig. Der geforderte Scope steht als `x-required-scope` an der Operation und zusaetzlich in ihrer `description`, damit er in Swagger UI sichtbar ist. **Nicht** ins Scope-Array des `security`-Eintrags: OpenAPI 3.0 erlaubt das nur bei `oauth2` und `openIdConnect`, bei `type: http` muss es leer bleiben. Backend-Routen und scope-freie Routen bekommen das Feld nicht. **Was weiter fehlt:** Response-Schemas pro Route (Listen liefern `{data, meta}`, Detail-Routen das Objekt flach).
...
- **Medienliste**: `filter[term]`, `filter[category_id_path]` und `filter[types]` bilden die Semantik von `rex_media_service::getList()` nach (`mediapool/lib/service_media.php:301-331`) — inklusive `type:jpg,png` im Suchbegriff und `"…"`-Gruppierung. Bewusst nachgebaut statt den Service zu rufen: `getList()` liefert nur Dateinamen und kennt die API-eigenen Bereichsfilter (Größe, Breite, Höhe) nicht, die Pagination würde damit auseinanderfallen. Änderungen an der Core-Semantik müssen hier nachgezogen werden.
...
- **Den richtigen Service treffen.** `rex_category_service` gehört zur Struktur und arbeitet auf `rex_article`; Medienkategorien laufen ausschließlich über `rex_media_category_service`. Die Namen sind fast gleich, die Tabellen und EPs nicht — ein aus `Structure.php` kopierter Handler löscht sonst Artikel statt Medienkategorien.
